...
export class CustomerService {
	...
	customers$ = (page: number = 0) =>
		<Observable<CustomHttpResponse<Page<Customer> & User & Stats>>>(
			this.http
				.get<CustomHttpResponse<Page<Customer> & User & Stats>>(`${this.server}/customer/list?page=${page}`)
	...
	customer$ = (customerId: number) =>
		<Observable<CustomHttpResponse<CustomerState>>>(
			this.http
				.get<CustomHttpResponse<CustomerState>>(`${this.server}/customer/get/${customerId}`)
				.pipe(tap(console.log), catchError(this.handleError))
		);
	...
	invoices$ = (page: number = 0) =>
		<Observable<CustomHttpResponse<Page<Invoice> & User>>>(
			this.http
				.get<CustomHttpResponse<Page<Invoice> & User>>(`${this.server}/customer/invoice/list?page=${page}`)
				.pipe(tap(console.log), catchError(this.handleError))
		);
	...
	invoice$ = (invoiceId: number) =>
		<Observable<CustomHttpResponse<Customer & Invoice & User>>>(
			this.http
				.get<CustomHttpResponse<Customer & Invoice & User>>(`${this.server}/customer/invoice/get/${invoiceId}`)
				.pipe(tap(console.log), catchError(this.handleError))
		);
